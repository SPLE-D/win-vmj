...
# Product: SuperBasedEventOrganizer
# Context: src-gen/SuperBasedEventOrganizer/ (same level as docker-compose.yml)
...
RUN mv eventmanager.product.superbasedeventorganizer/*.jar ./jars/ 2>/dev/null || true && \
...
# Run the application
CMD ["java", "-cp", "jars/*:.", "eventmanager.product.superbasedeventorganizer.SuperBasedEventOrganizer"]
