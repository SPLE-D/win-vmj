...
# Product: Fneo
# Context: src-gen/Fneo/ (same level as docker-compose.yml)
...
RUN mv eventmanager.product.fneo/*.jar ./jars/ 2>/dev/null || true && \
...
# Run the application
CMD ["java", "-cp", "jars/*:.", "eventmanager.product.fneo.Fneo"]
